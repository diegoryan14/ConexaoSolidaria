...
public interface AvaliacaoRepository extends JpaRepository<Avaliacao, Long>, JpaSpecificationExecutor<Avaliacao> {
    ...
    @Query(
        value = "select avaliacao from Avaliacao avaliacao left join fetch avaliacao.usuario left join fetch avaliacao.evento",
        countQuery = "select count(avaliacao) from Avaliacao avaliacao"
    )
    Page<Avaliacao> findAllWithToOneRelationships(Pageable pageable);

    @Query("select avaliacao from Avaliacao avaliacao left join fetch avaliacao.usuario left join fetch avaliacao.evento")
    List<Avaliacao> findAllWithToOneRelationships();

    @Query(
        "select avaliacao from Avaliacao avaliacao left join fetch avaliacao.usuario left join fetch avaliacao.evento where avaliacao.id =:id"
    )
