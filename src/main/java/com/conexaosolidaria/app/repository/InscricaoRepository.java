...
public interface InscricaoRepository extends JpaRepository<Inscricao, Long>, JpaSpecificationExecutor<Inscricao> {
    ...
    @Query(
        value = "select inscricao from Inscricao inscricao left join fetch inscricao.usuario left join fetch inscricao.evento",
        countQuery = "select count(inscricao) from Inscricao inscricao"
    )
    Page<Inscricao> findAllWithToOneRelationships(Pageable pageable);

    @Query("select inscricao from Inscricao inscricao left join fetch inscricao.usuario left join fetch inscricao.evento")
    List<Inscricao> findAllWithToOneRelationships();

    @Query(
        "select inscricao from Inscricao inscricao left join fetch inscricao.usuario left join fetch inscricao.evento where inscricao.id =:id"
    )
